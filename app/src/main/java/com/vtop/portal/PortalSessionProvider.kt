...
object PortalSessionProvider {
    ...
    ): Result<VtopClient> = withContext(Dispatchers.IO) {
        try {
                    ...
                    loginSuccess = authenticatingClient.autoLogin(
                        ...
                        object : VtopClient.LoginListener {
                            override fun onStatusUpdate(message: String) {
                                // Replaced GlobalScope leak with caller-provided suspension
                                kotlinx.coroutines.runBlocking { onStatusUpdate(message) }
                            }

                            override fun onOtpRequired(resolver: VtopClient.OtpResolver) {
                                // Launch detached job strictly for polling so main auth thread remains active
                                kotlinx.coroutines.CoroutineScope(Dispatchers.IO).launch {
                                    val otpRequestedTime = System.currentTimeMillis()
                                    val savedEmail = Vault.getGoogleEmail(context)

                                    if (savedEmail.isNotBlank()) {
                                        onStatusUpdate("Fetching OTP from Gmail...")
                                    ...
                                    onStatusUpdate("Awaiting manual OTP...")
